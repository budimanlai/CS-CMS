...
class ContentController extends MyController {
    ...
    */
    public function actionView($id) {
        ...
        $model->meta_keyword = Content::normalizeTagsRead($model->meta_keyword);
    ...
    public function actionCreate() {
        ...
        if(isset($_POST['Content'])) {
            ...
            if (!empty($this->category)) {
                $cc = ContentGroup::model()->find("seo_url = :SEO", array(
                    ':SEO' => $this->category
                ));
                if ($cc != null) { $_POST['Content']['group_id'] = $cc->id; }
            }
            $_POST['Content']['create_by'] = Yii::app()->user->id;
            $_POST['Content']['create_datetime'] = date("Y-m-d H:i:s");
            $_POST['Content']['meta_keyword'] = Content::normalizeTags($_POST['Content']['meta_keyword']);
            ...
            if($model->save()) {
                ...
                $this->redirect(array('view','id'=>$model->id, 'category'=>$model->mContentGroup->seo_url));
            }
        } else {
            if (!empty($this->category)) {
